...
import { getLeadAIInsights } from "../../services/aiService";
...
const AILeadIntelligence = () => {
  ...
  const loadLeadAI = useCallback(async () => {
    try {
      ...
      const response = await getLeadAIInsights();

      if (!response?.success) {
        throw new Error(
          response?.message || "Unable to load AI lead insights."
        );
      }

      setData(response.data || null);
    } catch (err) {
      console.error("Lead AI Insights Error:", err);

      setError(
        err?.message ||
          "Unable to load AI lead insights. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="mb-4 w-full rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-slate-500">
              Lead priorities, risks & follow-up recommendations
            </p>
          </div>
        </div>
      ...
      {!loading &&
        !error &&
        (!data || !Array.isArray(data.insights)) && (
          <div className="p-4">
            <div className="rounded-lg border border-dashed border-slate-200 p-4 text-center">
              <div className="text-xs font-medium text-slate-700">
                No AI lead insights available
              </div>

              <p className="mt-1 text-[11px] text-slate-500">
                Not enough lead activity for AI analysis yet.
              </p>
            </div>
          </div>
        )}
          ...
          data.insights.length > 0 ? (
            <div className="grid grid-cols-1 gap-2.5 lg:grid-cols-2">
              ...
              {data.insights.map((insight, index) => {
                ...
                return (
                  <article
                    key={`${insight?.title || "lead-insight"}-${index}`}
                      ...
                      insight.sourceRecords.length > 0 && (
                        <div className="mt-2 flex items-center justify-between">
                          <span className="text-[9px] font-medium text-slate-400">
                            Based on{" "}
                            {insight.sourceRecords.length} CRM{" "}
            ...
            <div className="rounded-lg border border-dashed border-slate-200 p-4 text-center">
              <p className="text-xs font-medium text-slate-700">
                No actionable lead insights right now
              </p>

              <p className="mt-1 text-[11px] leading-4 text-slate-500">
                AI will surface lead priorities, opportunities,
                risks and follow-up recommendations as activity
                becomes available.
              </p>
            </div>
          )}
